...
const char* addr_mode_templates[12] = {
  "(0x00%02x + X)", "0x00%02x", "$%02x", "0x%04x", "(0x00%02x) + Y", "0x00%02x + X", "0x%04x + Y", "0x%04x + X", "", "", "invalid", "+%2x"
};
...
const char* instrs[72] = {
  ...
  "BRK", "BPL", "JSR", "BMI", "RTI", "BVC", "RTS", "BVS", "invalid", "BCC", nullptr, "BCS", nullptr, "BNE", nullptr, "BEQ",
  
  // 16 0xX8 instructions
  "PHP", "CLC", "PLP", "SEC", "PHA", "CLI", "PLA", "SEI", "DEY", "TYA", "TAY", "CLV", "INY", "CLD", "INX", "SED",
  
  // 16 0xXA instructions
  "ASL A", "invalid", "ROL A", "invalid", "LSR A", "invalid", "ROR A", "invalid", "TXA", "TXS", "TAX", "TSX", "DEX", "invalid", "NOP", "invalid"
};
...
std::array<Op, 256> build_ops()  {
  ...
  for (int i = 0; i < 256; ++i) {
    if (i == 0x44 || i == 0x54 || i == 0x5c || i == 0x64 || i == 0x74 || i == 0x7c || i == 0x89 || i == 0x9c || i == 0x9e || i == 0xd4 || i == 0xdc || i == 0xf4 || i == 0xfc) {
