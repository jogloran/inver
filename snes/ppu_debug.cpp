...
void PPUDebug::dump_colour_math(const SPPU& sppu) {
...
void PPUDebug::dump_sprite(const SPPU& sppu) {
  word addr = 0x27c0 / 2;
  for (int i = 0; i < 32; ++i) {
...
void PPUDebug::dump_pal(const SPPU& sppu) {
...
void PPUDebug::dump_bg(const SPPU& sppu, byte layer) {
...
void PPUDebug::dump_oam_bytes(const SPPU& sppu) {
...
void PPUDebug::dump_oam_table(const SPPU& sppu) {
...
void PPUDebug::dump_oam(const SPPU& sppu, bool dump_bytes) {
  if (dump_bytes) {
    PPUDebug::dump_oam_bytes(sppu);
  } else {
    PPUDebug::dump_oam_table(sppu);
  }
}
